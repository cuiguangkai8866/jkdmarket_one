...
function getParentCategory($parent_id, $arr = array())
{
    if ($parent_id == 0) {
        return $arr;
    } else {
        $category = M('category')->where('id=' . $parent_id)->find();
        $arr = array_push($arr, $category['id']);
        $arr = array_merge(getParentCategory($category['parent_id']), $arr);
        return $arr;
    }
}
