...
function getPropValueByPropId($id)
{
    return M('prop_value')->where('prop_id=' . $id)->select();
}

function getCategoryById($id)
{
    return M('category')->where('id=' . $id)->find();
}

function getPropListByCategoryId($id)
{
    return M('product_prop')->where('category_id=' . $id)->order('is_sale_prop asc,sort desc')->select();
}

function getParentCategory($id, $arr = array())
{
    ...
    if ($category['parent_id'] == 0) {
        ...
        $arr = getParentCategory($category['parent_id'], $arr);
        return $arr;
    }
}

function getChildCategory($id, $arr = array())
{
    $categories = M('category')->where('parent_id=' . $id)->select();
    if(!$categories){
        return $arr;
    }
    foreach ($categories as $category) {
        array_push($arr,$category['id']);
       $arr=getChildCategory($category['id'],$arr);
    }
    return $arr;
}
function getNewsCategory($id){
   return  M('news_category')->where('id='.$id)->find();
}