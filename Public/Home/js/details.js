...
$(function() {
	...
	$(".show-list-next").bind('click', function() {
		if (hideNum != smallImg && curPos < hideNum) {
			...
			if (!$(".show-list-con").find("ul").is(":animated")) {
				$(".show-list-con ul").stop().animate({
					marginLeft : -smallWidth * curPos
				}, 250);
			}
		} else {
			curPos = hideNum;
	...
	$(".set-con span").live(
			...
			function() {
				...
				singlePri = singlePri.substr(1).substring(0,
						singlePri.length - 3);
				var cxPri = singlePri + "";
				if (cxPri.indexOf('.') == -1) {
					cxPri = cxPri + ".00";
				...
				$(".dPrice-pri p em").text(cxPri);
				jQuery("#_dmdyhtzyxz").html(
						"已选择<span>" + tzSum + "瓶&nbsp;&nbsp;￥" + singlePri
								+ "/瓶</span>").show();
				$('#_nub').val(tzSum);
			})
	/*// 购物数量加减
	...
	$(".buyNub-nub-top").bind(
			...
			function() {
				...
				var singlePri = $(".setOn em").text();
				singlePri = singlePri.substr(1).substring(0,
						singlePri.length - 3);// price
				var tzSum = $(".set-con").find("span").eq(buySum).find("b")
						.attr('nmb');
				...
				if (buySum >= 99) {
					...
					buySum = 99;
					$(".buyNub-nub").find("i").eq(1).addClass(
							"buyNub-nub-top-s");
				} else {
					$(".buyNub-nub").find("i").eq(1).removeClass(
							"buyNub-nub-top-s");
				}
				...
				if ($(".dOrder-set .set-con").length > 0) {
					for ( var i = 0; i < $(".dOrder-set span").length; i++) {
						...
						if (parseInt($(".set-con").find("span").eq(i).find("b")
								.attr('nmb')) == buySum) {
							$(".set-con").find("span").removeClass("setOn");
							$(".set-con").find("span").eq(i).addClass("setOn");
							var singlePri = $(".set-con").find("span").eq(i)
									.find("em").text();
							singlePri = singlePri.substr(1).substring(0,
									singlePri.length - 3);
							var cxPri = singlePri + "";
	...
	/*$(".buyNub-nub-blus-c")
			.bind(
					...
					function() {
						...
						;
						var reg = new RegExp("^[1-9][0-9]*$");
						var tzSum = $(".set-con").find("span").eq(buySum).find(
								"b").attr('nmb');
						// tzSum =
						// tzSum.substr(0,tzSum.length-1).substr(1);//number
						if (!reg.test(buySum)) {
							buySum = 1;
						} else {
							buySum--;
						...
						if (buySum <= 1) {
							buySum = 1;
							$(".buyNub-nub").find("i").eq(0).addClass(
									"buyNub-nub-blus");
						}
						$(".buyNub-nub").find('input').val(buySum);
						if ($(".dOrder-set .set-con").length > 0) {
							...
							for ( var i = 0; i < $(".dOrder-set span").length; i++) {
								...
								if (parseInt($(".set-con").find("span").eq(0)
										.find("b").attr('nmb')) > buySum) {
									$(".set-con").find("span").removeClass(
											"setOn");
								} else {
									if (parseInt($(".set-con").find("span").eq(
											i).find("b").attr('nmb')) <= buySum) {
										$(".set-con").find("span").removeClass(
												"setOn");
										$(".set-con").find("span").eq(i)
												.addClass("setOn");
										singlePri = $(".set-con").find("span")
												.eq(i).find("em").text();
										singlePri = singlePri.substr(1)
												.substring(0,
														singlePri.length - 3);
	...
	for (nRank = 0; nRank <= 2; nRank++) {
		$(".dLeft-ranking li").eq(nRank).find(".rUnfold-nub2").attr('class',
				'rUnfold-nub');
	}
	$(".dLeft-ranking li").last().children(".rank-packup").css('border-bottom',
			'none');
	$(".dLeft-ranking li").last().children(".rank-unfold").css('border-bottom',
			'none');
	...
	// 商品回答
	$(".u-consulting-title li").bind(
			'click',
			function() {
				var ansLs = $(".u-consulting-title li").index($(this));
				$(".u-consulting-title li").removeClass("on");
				$(this).addClass("on");
				$(".u-consulting-info-box").find(".u-consulting-info-con")
						.hide();
				$(".u-consulting-info-box").find(".u-consulting-info-con").eq(
						ansLs).show();
				zxtype = ansLs;
				initzx(ansLs);
			});
	...
	$(
			"#u-buy-layId .u-buy-close,#u-buy-layId .u-buy-g,#u-buy-layId2 .u-buy-close,#u-buy-layId2 .u-buy-g")
			.click(function() {
				$("#u-buy-layId").hide();
				$("#u-buy-layId2").hide();
	...
	$(
			"#u-buy-laycl .u-buy-close,#u-buy-laycl2 .u-buy-close,.u-buy-gon-small .u-buy-g")
			.click(function() {
				$("#u-buy-laycl").hide();
				$("#u-buy-laycl2").hide();
			});
	...
	// 最终购买弹出层
	$(
			"#u-buy-left_one .u-buy-close,#u-buy-left_one .u-buy-g, #u-buy-left_two .u-buy-g, #u-buy-left_two .u-buy-close")
			.live('click', function() {
				$('#u-buy-left_one').hide();
				$('#u-buy-left_two').hide();
